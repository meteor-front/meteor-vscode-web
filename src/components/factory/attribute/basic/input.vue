    ...
    <el-tabs v-model="attrTab" @tab-click="handleTabClick">
      <el-tab-pane label="属性" name="1">
        <div class="container-attribute-box">
          <attribute-item v-for="(attr, key) in attributeForm" :key="key" :name="key" :title="attr.title" :type="attr.type" :option-type="attr.optionType" :options="widgetOpitons[attribute.type][key]" />
        </div>
      </el-tab-pane>
      <el-tab-pane label="插槽" name="2">
        <slot-item v-for="(attr, key) in slotForm" :key="key" :name="key" :title="attr.title" :type="attr.type" :option-type="attr.optionType" :options="widgetOpitons[attribute.type][key]" />
      </el-tab-pane>
...
import SlotItem from '@/components/factory/attribute/sub/slotItem'
...
export default {
  name: 'WidgetBasicInput',
  components: { AttributeItem, SlotItem, styleItem },
  props: {
    attribute: {
      type: Object,
      default: function() {

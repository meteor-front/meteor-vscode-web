<template>
  <div class="zl-page" :class="$store.state.mode === '1' ? 'simple' : ''">
    <search ref="search" :tag-list="tagList" tab="4" @fetchList="fetchList" @showHandlerPop="showHandlerPop" />
...
export default {
  ...
  methods: {
    ...
    fetchList() {
      ...
      this.loading = true
      request.get('widget?tag=' + form.tag + '&searchValue=' + form.searchValue + '&common=1').then((res) => {
